...
async function run() {
  try {
    ...
    const r1 = await axios.post(
      base,
      { model: "qwen-max-latest", messages: [{ role: "user", content: "Меня зовут Дима" }] },
    ...
    const r2 = await axios.post(
      base,
      { model: "qwen-max-latest", messages: [{ role: "user", content: "Как меня зовут?" }] },
      { headers, timeout: 120000 }
    );
    ...
    try {
      ...
      const matches = logs
        ...
        .filter(
          (l) =>
            l.includes("[raw]") || l.includes("rawChunks") || l.includes("Ответ получен успешно")
        );
