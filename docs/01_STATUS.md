...
| Agent-loop stability | Working | Qwen: deferred auto-reset, cooldown, same-chat retry on "in progress". Anti-loop guard: cross-turn repeat detection (tool_call_id matching) for read-only tools only + anti-thinking-loop (>3 sequentialthinking). Destructive operations and failed tool results are excluded from repeat detection. Covers both stream and non-stream paths. DeepSeek: N/A (single-message model). |
