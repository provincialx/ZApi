...
function addMessageToHistory(chatId, message) {
  try {
    ...
    if (chatData.messages.length >= MAX_HISTORY_LENGTH) {
      logInfo(
        `Чат ${chatId} достиг максимальной длины (${MAX_HISTORY_LENGTH}), удаляем старые сообщения`
      );
      chatData.messages = [
        chatData.messages[0],
        ...chatData.messages.slice(chatData.messages.length - MAX_HISTORY_LENGTH + 2),
      ];
    }
      ...
      .map((file) => {
        ...
        return {
          ...
          userMessageCount: chatData.messages
            ? chatData.messages.filter((m) => m.role === "user").length
            : 0,
        };
      });
...
export function deleteChatsAutomatically(criteria = {}) {
  try {
    ...
    if (olderThan) {
      ...
      logInfo(
        `Чатов старше ${olderThan}мс (${new Date(cutoffTime).toLocaleString()}): ${oldChatsCount}`
      );
      chatsToDelete = chatsToDelete.filter((chat) => chat.created < cutoffTime);
    }

    if (userMessageCountLessThan !== undefined) {
      const lowUserMsgChatsCount = chatsToDelete.filter(
        (chat) => chat.userMessageCount < userMessageCountLessThan
      ).length;
      logInfo(
        `Чатов с менее чем ${userMessageCountLessThan} сообщений пользователя: ${lowUserMsgChatsCount}`
      );
      chatsToDelete = chatsToDelete.filter(
        (chat) => chat.userMessageCount < userMessageCountLessThan
      );
    }

    if (messageCountLessThan !== undefined) {
      const lowMsgChatsCount = chatsToDelete.filter(
        (chat) => chat.messageCount < messageCountLessThan
      ).length;
      logInfo(`Чатов с менее чем ${messageCountLessThan} сообщений всего: ${lowMsgChatsCount}`);
      chatsToDelete = chatsToDelete.filter((chat) => chat.messageCount < messageCountLessThan);
    }

    if (maxChats && chats.length > maxChats) {
      logInfo(
        `Общее количество чатов (${chats.length}) превышает лимит (${maxChats}), удаляем старые чаты`
      );
